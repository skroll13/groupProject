...
const moonMain = async dateOutput => {
  try {
    let fetchData = await fetch(
      `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/Kansas/${dateOutput}?unitGroup=us&key=PXLCYVMHK8W88EN9J7UFB2NFN&include=days&elements=moonphase,sunrise,sunset&contentType=json`
    )
...
const astroMain = async data => {
  try {
    let fetchAstroData = await fetch(
      `https://ohmanda.com/api/horoscope/${data}`
    )
